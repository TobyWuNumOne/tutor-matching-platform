  ...
  <main class="pt-[110px] min-h-screen flex flex-col">
    ...
    <section class="p-6 flex items-center space-x-6 bg-gray-50 flex-wrap">
      <select v-model="selectedCategory"
        ...
        <option v-for="(item, i) in CourseCategories" :key="i" :value="item">{{ item }}</option>
      </select>

      <select v-model="selectedRating"
        class="bg-white text-gray-700 text-sm sm:text-lg border border-gray-300 rounded px-4 py-2 shadow-md max-w-xs hover:cursor-pointer focus:outline-none">
        <option disabled value="">請選擇評價</option>
        <option v-for="(star, i) in Rating" :key="i" :value="star">{{ star }}</option>
      </select>

      <p class="text-gray-700 text-base sm:text-lg flex-grow text-center min-w-[200px]">找到最適合你的導師，開始學習吧！</p>
    </section>

    <!-- 導師卡片區 -->
    <section class="flex-1 p-6 bg-gray-100 grid grid-cols-2 gap-4 sm:grid-cols-4 overflow-x-auto">
      <div v-for="(teacher, index) in Teachers" :key="index"
        ...
        <img :src="`https://source.unsplash.com/random/200x200?sig=${index}`" alt="老師照片"
        ...
        <div class="flex space-x-1 mb-4">
          <img v-for="starIndex in teacher.rating" :key="starIndex + '-' + index" src="../assets/star_icon.png"
            alt="star" class="w-5 h-5" />
        </div>

        <!-- 預約按鈕 -->
        <button class="bg-[#3F3FF0] text-white px-4 py-2 rounded hover:bg-blue-700 cursor-pointer transition"
          @click="openModal(index)">
          預約老師
        </button>

        <!-- Modal 區塊 -->
        <div v-if="activeModalIndex === index"
          class="fixed inset-0 bg-white/30 backdrop-blur-md flex justify-center items-center z-50">
          <div v-if="login" class="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full relative border border-gray-200">
            <button class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 cursor-pointer" @click="closeModal">✕</button>
            <h2 class="text-xl font-bold mb-4">預約：{{ teacher.name }}</h2>
            <p class="mb-4 text-gray-600">您可以選擇以下動作：</p>
            <div class="flex flex-col space-y-2">
              <button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 cursor-pointer">確認預約</button>
              <button class="bg-gray-300 text-black px-4 py-2 rounded hover:bg-gray-400 cursor-pointer">發送訊息</button>
              <button class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 cursor-pointer" @click="closeModal">取消</button>
            </div>
          </div>
          ...
          <div v-else
            ...
            <button class="absolute top-2 right-2 text-gray-500 hover:text-gray-700 cursor-pointer" @click="closeModal">✕</button>
            <p class="mb-4 text-lg font-semibold text-red-600">您尚未登入，請先登入</p>
            <RouterLink to="/login">
              <button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 cursor-pointer">前往登入</button>
            </RouterLink>
          </div>
        </div>
      </div>
