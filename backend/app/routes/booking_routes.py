            ...
            400: {
                ...
                "schema": {
                    ...
                    "properties": {
                        ...
                        "errors": {
                            "type": "object",
                            "example": {"schedule_date": ["預約時間格式錯誤"]},
                        },
                    },
                },
            },
...
def create_booking():
    ...
    try:
        ...
        if not student:
            return jsonify({"success": False, "message": "指定的學生不存在"}), 404
        ...
        if existing_booking:
            return jsonify({"success": False, "message": "該時段已有其他預約"}), 409
        ...
        booking_with_relations = Booking.query.options(
            db.joinedload(Booking.course), db.joinedload(Booking.student)
        ).get(new_booking.id)
        ...
        return (
            jsonify(
                {"success": True, "message": "預約申請已送出", "data": response_data}
            ),
            201,
        )
        ...
        db.session.rollback()
        return jsonify({"success": False, "message": f"預約建立失敗: {str(e)}"}), 500
...
def get_booking_list():
    ...
    try:
        ...
        return (
            jsonify({"success": False, "message": f"獲取預約列表失敗: {str(e)}"}),
            500,
        )
...
def get_booking_detail(booking_id):
    ...
    try:
        ...
        if not booking:
            return jsonify({"success": False, "message": "找不到指定的預約"}), 404
        ...
        return (
            jsonify({"success": False, "message": f"獲取預約詳細資訊失敗: {str(e)}"}),
            500,
        )
...
def update_booking(current_user, booking_id):
    ...
    try:
        ...
        # 檢查權限（老師或學生可以更新相關的預約）
        is_teacher = (
            hasattr(current_user, "teacher")
            and current_user.teacher
            and booking.course.teacher_id == current_user.teacher.id
        )
        is_student = (
            hasattr(current_user, "student")
            and current_user.student
            and booking.student_id == current_user.student.id
        )

        if not (is_teacher or is_student):
            return jsonify({"success": False, "message": "您沒有權限修改此預約"}), 403
        ...
        return (
            jsonify(
                {"success": True, "message": "預約狀態更新成功", "data": response_data}
            ),
            200,
        )
        ...
        db.session.rollback()
        return jsonify({"success": False, "message": f"預約更新失敗: {str(e)}"}), 500
...
def delete_booking(current_user, booking_id):
    ...
    try:
        ...
        if not (is_teacher or is_student):
            return jsonify({"success": False, "message": "您沒有權限刪除此預約"}), 403
        ...
        db.session.rollback()
        return jsonify({"success": False, "message": f"預約刪除失敗: {str(e)}"}), 500
        ...
        "responses": {
            200: {
                ...
                "schema": {
                    ...
                    "properties": {
                        ...
                        "data": {
                            "type": "array",
                            "items": {"$ref": "#/definitions/BookingResponse"},
                        },
                    },
                },
            },
            404: {
                ...
                "schema": {
                    ...
                    "properties": {
                        "success": {"type": "boolean", "example": False},
                        "message": {"type": "string", "example": "學生不存在"},
                    },
                },
            },
        },
...
@auth_required
def get_bookings_by_student(student_id):
    """
    根據學生ID獲取預約
    """
    ...
    booking_response_schema = BookingResponseSchema(many=True)
    response_data = booking_response_schema.dump(bookings)
