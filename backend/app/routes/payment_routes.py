...
def convert_to_ecpay_params(order_data):
    ...
    try:
        ...
        ecpay_params = {
            ...
            "ItemURL": "http://localhost:5173",  # 商品資訊頁面
            "Remark": f"老師ID: {teacher_id_display}",
            "ChooseSubPayment": "",
            # 🎯 付款完成後的跳轉頁面（帶上訂單編號）
            "ClientBackURL": f"http://localhost:5173/teacherdashboard",
            "OrderResultURL": f"http://localhost:5173/teacherdashboard",
            "NeedExtraPaidInfo": "Y",
            "DeviceSource": "",
            "IgnorePayment": "",
            "PlatformID": "",
...
@swag_from(
    {
        ...
        "responses": {
            200: {
                ...
                "schema": {"type": "string", "example": "1|OK"},
            }
        },
    }
)
...
def verify_check_mac_value(form_data):
    ...
    try:
        ...
        print(
            f"   主要參數: MerchantID={params_copy.get('MerchantID')}, TradeNo={params_copy.get('TradeNo')}"
        )
...
@swag_from(
    {
        ...
        "responses": {
            ...
            400: {
                ...
                "schema": {"type": "string", "example": "0|缺少必要欄位"},
            },
            500: {
                "description": "伺服器內部錯誤",
                "schema": {"type": "string", "example": "0|處理錯誤"},
            },
        },
    }
)
            ...
            try:
                if verify_check_mac_value(form_data):
                    ...
                    update_payment_status(
                        merchant_trade_no, verification_status, form_data
                    )
                    print(
                        f"🔵 付款成功，已啟用老師 {payment_record.teacher_id} 的藍勾勾認證"
                    )
                else:
