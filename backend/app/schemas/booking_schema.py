...
class BookingCreateSchema(Schema):
    """用於創建預約的 Schema - 使用純 Marshmallow Schema"""
    ...
    schedule_date = fields.String(required=True)  # 改為字串格式
...
class BookingResponseSchema(SQLAlchemyAutoSchema):
    ...
    def get_teacher_name(self, obj):
        return (
            obj.course.teacher.user.name
            if obj.course and obj.course.teacher and obj.course.teacher.user
            else None
        )
