...
<template>
    <nav class="fixed top-0 w-full h-[80px] bg-[#3F3FF0] flex justify-between items-center px-4 z-50">
        <!-- 左側 logo -->
        <div class="flex items-center gap-2 sm:gap-5">
            <img src="../assets/customer-service-headset.png" class="w-10 h-10 invert" />
        ...
        <!-- 手機版漢堡選單 -->
        <button class="md:hidden text-white" @click="toggleMobileMenu" aria-label="Toggle Menu">
        ...
        <div v-if="showMenu"
            ...
            <div class="flex flex-col text-[#3F3FF0] font-medium">
                ...
                <RouterLink to="/personaldashboard" v-if="role === 'student' && $route.path !== '/'"
                    class="px-4 py-3 hover:bg-[#f0f4ff] transition">
                    學生個人頁面
                </RouterLink>

                <RouterLink to="/teacherdashboard" v-else-if="role === 'teacher' && $route.path !== '/'"
                    class="px-4 py-3 hover:bg-[#f0f4ff] transition">
                    教師個人頁面
                </RouterLink>

                <RouterLink to="/" v-else-if="role === 'admin' && $route.path !== '/'"
                    class="px-4 py-3 hover:bg-[#f0f4ff] transition">
                    管理者頁面
                </RouterLink>
