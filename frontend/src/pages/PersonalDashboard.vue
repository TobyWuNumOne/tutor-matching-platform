<script setup>
    ...
    const fetchUserProfile = async () => {
        ...
        try {
            ...
            // 無論角色為何，皆執行 fetchStudentDetails
    ...
    // 獲取學生詳細資料 (自動從JWT獲取用戶ID)
    const fetchStudentDetails = async (userId) => {
        try {
            console.log('🔍 根據用戶 ID 獲取學生詳細資料...');
            ...
            if (response.data.success) {
                ...
                students.gender =
                    studentData.gender || students.gender || '尚未設定';
                students.age = studentData.age || students.age || '尚未設定';
                ...
                students.country = '臺北 Taipei';
                students.specialization = '學生';
                students.gender = students.gender || '尚未設定';
                students.age = students.age || '尚未設定';
            }
        } catch (err) {
    ...
    const fetchUserBookings = async () => {
        ...
        try {
            console.log('🔍 獲取用戶預約資料...');

            const response = await bookingAPI.getBookingsByStudentId(
                students.id
            );
            if (response.data.success) {
                const bookings = response.data.data;
                bookedTeachers.value = bookings.map((booking) => ({
                    id: booking.id,
                ...
                console.log('✅ 預約資料:', bookedTeachers.value);
            } else {
                bookingsError.value = '無法取得預約資料';
            ...
            >
                ...
                <div class="md:col-span-7 space-y-6">
                    ...
                    <div class="bg-white p-4 rounded-lg shadow space-y-4">
                        ...
                        <div
                            v-if="
                                bookingsLoading && bookedTeachers.length === 0
                            "
                        ...
                        <!-- 無預約資料 -->
                        <div
                            v-else-if="bookedTeachers.length === 0"
